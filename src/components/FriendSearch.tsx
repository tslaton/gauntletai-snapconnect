...
import { type UserSearchResult as UserResult } from '../utils/friendsApi';
...
export default function FriendSearch() {
  ...
  }, [searchQuery, currentUser?.id, fetchSearchResults, clearSearch]);
  ...
  const renderUserResult = ({ item }: { item: UserResult }) => {
    ...
    const requestStatus = getRequestStatus(item.id, currentUser.id);
      ...
      return (
        <View className="flex-1 items-center justify-center px-6">
          ...
          <Text className="text-gray-500 mt-2 text-center">
            Search for friends by username or name to connect with them
          </Text>
        </View>
      );
    }
